...
func deleteInferenceRouter(t *testing.T, routerUUID string) {
	...
	if resp != nil && resp.IsError {
		t.Logf("inference router cleanup delete returned error: %v", resp.Content)
	}
}
...
func TestInferenceRouterCreateDeleteLifecycle(t *testing.T) {
	...
	del := callTool[deleteModelRouterResponse](t, "genai-inference-router-delete", map[string]interface{}{
		"UUID": routerUUID,
	})
